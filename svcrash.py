...
def getArgs():
    ...
    parser.add_option(
        '-d',
        help="specify attacker's ip address",
        dest="ipaddr",
    )
    ...
    parser.add_option(
        '-b',
        help="bruteforce the attacker's port",
        dest="bruteforceport",
        default=False,
        action="store_true")
...
class asteriskreadlognsend:
    ...
    def findfailures(self):
        ...
        if time.time() - self.lastsent <= 2:
            return
        match = re.search(
            "Registration from '(.*?)' failed for '(.*?)' - (No matching peer found|Wrong password)",
            buff)
        if match:
            self.matchcount += 1
