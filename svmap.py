...
class DrinkOrSip:
    ...
    def __init__(
        ...
        if self.sessionpath is not None:
            self.resultip = dbm.ndbm.open(
                os.path.join(self.sessionpath, 'resultip'), 'c')
            self.resultua = dbm.ndbm.open(
                os.path.join(self.sessionpath, 'resultua'), 'c')
            self.resultfp = dbm.ndbm.open(
    ...
    def getResponse(self, buff, srcaddr):
        ...
        if buff.startswith('OPTIONS ') \
            ...
            if self.externalip == srcip:
                self.log.debug(
                    "We received our own packet from %s:%s" %
                    srcaddr)
            else:
                self.log.info(
        ...
        if res is not None:
            ...
            self.log.info(
                'Packet from %s:%s did not contain a SIP msg' %
                srcaddr)
        ...
        while True:
            ...
            if r:
                ...
                localtag = createTag('%s%s' % (''.join(
                    ['%02x' % int(x) for x in dsthost[0].split('.')]), '%04x' % dsthost[1]))
                cseq = 1
...
if __name__ == '__main__':
    ...
    parser.add_option(
        ...
        "--input",
        dest="input",
        help="Scan IPs which were found in a previous scan. Pass the session name as the argument",
        metavar="scan1")
    parser.add_option(
        "-I",
        "--inputtext",
        dest="inputtext",
        help="Scan IPs from a text file - use the same syntax as command line but with new lines instead of commas. Pass the file name as the argument",
        metavar="scan1")
    parser.add_option(
        "-m",
        "--method",
    ...
    if options.resume is not None:
        ...
        if not os.path.exists(exportpath):
            logging.critical(
                'A session with the name %s was not found' %
                options.resume)
            exit(1)
    ...
    if options.input is not None:
        ...
        if options.randomize:
            ...
            if options.save is not None:
                scanrandomstore = os.path.join(exportpath, 'random')
                resumescan = True
            scaniter = scanrandom(list(map(getranges, args)), portrange, options.method.split(
                ','), randomstore=scanrandomstore, resume=resumescan)
        else:
    ...
    if options.save is not None:
        if options.resume is None:
            ...
            try:
                ...
                logging.critical(
                    'could not create the export location %s' %
                    exportpath)
                exit(1)
        ...
        if lenres > 0:
            ...
            if (lenres < 400 and options.save is not None) or options.save is None:
                ...
                for k in list(sipvicious.resultua.keys()):
                    rows.append(
                        (k, sipvicious.resultua[k], sipvicious.resultfp[k]))
                print((indent([labels] + rows,
                              hasHeader=True,
                              prefix='| ',
                              postfix=' |',
                              wrapfunc=lambda x: wrap_onspace(x,
                                                              width))))
            else:
