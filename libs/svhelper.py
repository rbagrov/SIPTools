...
def standardoptions(parser):
    ...
    parser.add_option(
        ...
        dest="port",
        default="5060",
        help="Destination port or port ranges of the SIP device - eg -p5060,5061,8000-8100",
        metavar="PORT")
    parser.add_option(
        "-P",
        "--localport",
        ...
        default=5060,
        type="int",
        help="Source port for our packets",
        metavar="PORT")
    parser.add_option(
        "-x",
        "--externalip",
        dest="externalip",
        help="IP Address to use as the external ip. Specify this if you have multiple interfaces or if you are behind NAT",
        metavar="IP")
    parser.add_option(
        "-b",
        "--bindingip",
...
def getRange(rangestr):
    ...
    for _tmp2 in _tmp1:
        ...
        if len(_tmp3) > 1:
            ...
            endport += 1
            numericrange.append(list(range(startport, endport)))
        else:
    ...
    if len(headerlines) > 1:
        ...
        if type == 'response':
            ...
            if len(_t) == 3:
                ...
                log.warn(
                    'Could not parse the first header line: %s' %
                    repr(_t))
                return r
            try:
                r['code'] = int(_code)
...
def makeRedirect(previousHeaders, rediraddr):
    ...
    r = response
    for h in list(superheaders.items()):
        r += '%s: %s\r\n' % h
    for h in list(headers.items()):
        r += '%s: %s\r\n' % h
    r += '\r\n'
    return(r)
...
def makeRequest(
    ...
    if auth is not None:
        response = challengeResponse(auth['username'], auth['realm'], auth[
                                     'password'], method, uri, auth['nonce'])
        if auth['proxy']:
    ...
    for h in list(headers.items()):
        r += '%s: %s\r\n' % h
    for h in list(finalheaders.items()):
        r += '%s: %s\r\n' % h
    r += '\r\n'
    r += body
    return(r)
...
def reportBugToAuthor(trace):
    ...
    data += '\r\n'
    data += 'email: <%s>' % eval(input("Your email address (optional): "))
    data += '\r\n'
    data += 'msg: %s' % eval(input("Extra details (optional): "))
    data += '\r\n'
    data += "python version: \r\n"
    data += "%s\r\n" % version
    ...
    data = quote(data)
    try:
        urlopen('http://geekbazaar.org/bugreport/r2.php',
                urlencode({'d': data}))
        ...
        for r in listresult[k]:
            ...
            if count:
                ...
                if not os.path.exists(dbloc):
                    logging.debug(
                        'The database could not be found: %s' %
                        dbloc)
                else:
                    db = dbm.ndbm.open(dbloc, 'r')
                    dblen = len(db)
...
def createReverseLookup(src, dst):
    ...
    if len(srcdb) > 100:
        log.warn(
            "Performing dns lookup on %s hosts. To disable reverse ip resolution make use of the -n option" %
            len(srcdb))
...
def getasciitable(labels, db, resdb=None, width=60):
    ...
    o = indent(
        ...
        prefix='| ',
        postfix=' |',
        wrapfunc=lambda x: wrap_onspace(
            x,
            width))
    return o
