...
#   SIP response codes, also mapped to ISDN Q.931 disconnect causes.
        ...
        if self.enableack:
            ...
            if 699 > _tmp['code'] >= 200:
                ...
                try:
                    ...
                    self.log.warn(
                        'could not parse the from address %s' %
                        _tmp['headers']['from'])
                    username = 'XXX'
                ...
                if 'INVITE' == cseqmethod:
                    ...
                    ackreq = self.createRequest(
                        'ACK', cid=cid, cseq=cseq.replace(
                            cseqmethod, ''), fromaddr=fromaddr, toaddr=toaddr, )
                    ...
                    if _tmp['code'] == 200:
                        ...
                        mysendto(
                            self.sock, byemsg, (self.dsthost, self.dstport))
    ...
    def start(self):
        ...
        try:
            ...
            if gotbadresponse:
                self.log.error(
                    "The response we got was not good: %s" %
                    repr(buff))
            else:
                self.log.error(
        ...
        if self.BADUSER.startswith(self.AUTHREQ):
            self.log.warn(
                "Bad user = %s - svwar will probably not work!" %
                self.AUTHREQ)
                        ...
                        if next(self.packetcount):
                            try:
                                if self.guessmode == 1:
                                    ...
                                    pickle.dump(
                                        self.guessargs.tell(), open(
                                            os.path.join(
                                                exportpath, 'lastextension.pkl'), 'w'))
                                    self.log.debug(
                                        'logged last position %s' %
                                        self.guessargs.tell())
                            except IOError:
                                self.log.warn(
